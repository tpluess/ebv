...
Msg const *MainState_ShowErosion(MainState *me, Msg *msg)
{
	switch (msg->evt)
	{
	case IPC_GET_NEW_IMG_EVT:
	{
		/* Write out the current gray image to the address space of the CGI. */
		memcpy(data.ipc.req.pAddr, data.u8TempImage[EROSION], sizeof(data.u8TempImage[EROSION]));
...
Msg const *MainState_ShowDilation(MainState *me, Msg *msg)
{
	switch (msg->evt)
	{
	case IPC_GET_NEW_IMG_EVT:
...
Msg const *MainState_ShowLabel(MainState *me, Msg *msg)
{
	switch (msg->evt)
	{
	case IPC_GET_NEW_IMG_EVT:
	{
		/* Write out the current gray image to the address space of the CGI. */
		memcpy(data.ipc.req.pAddr, data.u8TempImage[LABELIMG], sizeof(data.u8TempImage[LABELIMG]));
...
void MainStateConstruct(MainState *me)
{
	...
	StateCtor(&me->showthresh, "Show Threshold", &((Hsm *)me)->top, (EvtHndlr)MainState_ShowThreshold);
	StateCtor(&me->showerosion, "Show Erosion", &((Hsm *)me)->top, (EvtHndlr)MainState_ShowErosion);
  StateCtor(&me->showdilation, "Show Dilation", &((Hsm *)me)->top, (EvtHndlr)MainState_ShowDilation);
  StateCtor(&me->showlabel, "Show Labels", &((Hsm *)me)->top, (EvtHndlr)MainState_ShowLabel);
}
